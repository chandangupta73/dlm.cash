...
class USDTWalletAdmin(admin.ModelAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        # Add security section only for superusers
        if request.user.is_superuser:
            fieldsets += (
                ('Security Information', {
                    'fields': ('private_key_encrypted', 'decrypted_private_key'),
                    'classes': ('collapse',),
                    'description': 'Private key information (superuser only).'
                }),
            )
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if request.user.is_superuser:
            fields.append('decrypted_private_key')
        return fields
    
    def decrypted_private_key(self, obj):
        """Display decrypted private key for superusers only."""
        if not obj.private_key_encrypted:
            return "No private key"
        try:
            wallet_service = RealWalletService()
            decrypted_key = wallet_service.decrypt_private_key(obj.private_key_encrypted)
            return format_html(
